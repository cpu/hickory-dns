...
use hickory_proto::rr::record_type::RecordType as ProtoRecordType;
...
fn pdns_opportunistic_dot_success() -> Result<()> {
        ...
        |captures| {
            captures.iter().any(|c| {
                matches!(c.direction, Direction::Incoming { .. })
                    && c.dst_port == DOT_PORT
                    && query_name_and_type(c) == ("<Root>", ProtoRecordType::MX)
            })
        },
        Duration::from_secs(10),
    )?;
    ...
    assert_eq!(
        ...
        ("<Root>", ProtoRecordType::A)
    );
    assert_eq!(
        query_name_and_type(&udp_queries[1]),
        ("<Root>", ProtoRecordType::AAAA)
    );
...
fn query_name_and_type(c: &Capture) -> (&str, ProtoRecordType) {
    ...
    let r#type = ProtoRecordType::from(
        first_query
            .get("dns.qry.type")
            .unwrap()
            .as_str()
