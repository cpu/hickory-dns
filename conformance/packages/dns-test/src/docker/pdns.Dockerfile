...
RUN apt-get update && \
    ...
    echo "deb [signed-by=/etc/apt/keyrings/rec-52-pub.asc] http://repo.powerdns.com/debian bookworm-rec-52 main" > /etc/apt/sources.list.d/pdns-rec.list && \
    echo "deb [signed-by=/etc/apt/keyrings/auth-49-pub.asc] http://repo.powerdns.com/debian bookworm-auth-49 main" > /etc/apt/sources.list.d/pdns-auth.list && \
