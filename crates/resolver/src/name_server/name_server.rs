...
impl<P: ConnectionProvider> NameServer<P> {
    ...
    pub(super) async fn send(
        ...
        let (handle, meta, protocol) = self.connected_mut_client(preferences).await?;
        let now = Instant::now();
...
pub(super) struct ConnectionState<P: ConnectionProvider> {
    pub(super) protocol: Protocol,
    pub(super) handle: P::Conn,
    pub(super) meta: Arc<ConnectionMeta>,
}
