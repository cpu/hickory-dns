...
mod tests {
    ...
    use crate::{
        runtime::TokioRuntimeProvider, tests::udp_client_stream_bad_id_test,
        tests::udp_client_stream_response_limit_test, tests::udp_client_stream_test,
    };
    ...
    #[tokio::test]
    async fn test_udp_client_stream_bad_id() {
        subscribe();
        let provider = TokioRuntimeProvider::new();
        udp_client_stream_bad_id_test(IpAddr::V4(Ipv4Addr::LOCALHOST), provider).await;
    }

    #[tokio::test]
    async fn test_udp_client_stream_response_limit() {
        subscribe();
        let provider = TokioRuntimeProvider::new();
        udp_client_stream_response_limit_test(IpAddr::V4(Ipv4Addr::LOCALHOST), provider).await;
    }
}
